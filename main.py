...
class App(ctk.CTk):
        ...
        if TrashClassifier:
            try:
                ...
                if not os.path.exists(model_path):
                    check_path = "waste_project/yolov12_run/weights/best.pt"
                    if os.path.exists(check_path):
                        model_path = check_path
                
                print(f"Loading main model from: {model_path}")
                
                self.ai = TrashClassifier(
                    classification_model_path=model_path,
                    detection_model_path=None, # Tự tải yolov12n
                    use_hybrid=True
                )
    ...
    def update_status_panel(self):
        if self.ai and getattr(self.ai, 'classification_model', None):
            self.lbl_model_status.configure(text="● Model: Ready (Hybrid)", text_color=COLORS["primary"])
        else:
            self.lbl_model_status.configure(text="● Model: Error/Loading", text_color=COLORS["danger"])
    ...
    def open_image(self):
        ...
        if path and self.ai:
            try:
                ...
                # 2. Dùng Hybrid Detection
                detections = self.ai.detect_and_classify(img_cv, conf_threshold=0.25)
                ...
                if detections:
                    # Vẽ khung lên ảnh
                    img_cv = self.ai.draw_bounding_boxes(img_cv, detections)
                    
                    # Lấy kết quả tốt nhất
                    best = max(detections, key=lambda x: x[1])
                    final_label, final_conf = best[0], best[1]
                    res_text = f"FOUND {len(detections)} OBJECTS"
                else:
                    # Fallback quét toàn ảnh
                    final_label, final_conf = self.ai.predict_frame(img_cv)
                    res_text = f"SCENE: {final_label.upper()}"
                
                # 3. Hiển thị UI
                self.last_result = f"{final_label.upper()}\n({final_conf:.1%})"
                self.lbl_up_res.configure(text=res_text)
                self.update_status_panel()
                
                # 4. Hiển thị ảnh kết quả (đã vẽ khung)
                img_res = cv2.cvtColor(img_cv, cv2.COLOR_BGR2RGB)
                img_res_pil = Image.fromarray(img_res)
                
                # Resize thông minh để vừa khung
                img_res_pil.thumbnail((800, 600))
                
                self.lbl_up_img.configure(image=ctk.CTkImage(img_res_pil, size=img_res_pil.size), text="")
                self.lbl_up_img.place(relx=0.5, rely=0.5, anchor="center")
    ...
    def loop_camera(self):
        ...
        if self.ai:
            detections = self.ai.detect_and_classify(frame, conf_threshold=0.3)
            
            if detections:
                # 1. Vẽ khung các vật thể tìm thấy
                frame = self.ai.draw_bounding_boxes(frame, detections)
                
                # 2. Cập nhật Status Panel với vật thể rõ nhất
                best_det = max(detections, key=lambda x: x[1])
                label, conf = best_det[0], best_det[1]
                
                if conf > 0.5:
                    self.last_result = f"{label.upper()} ({conf:.1%})"
                else:
                    self.last_result = "Uncertain..."
            else:
                # Fallback: Không thấy vật thể -> Quét toàn màn hình
                label, conf = self.ai.predict_frame(frame)
                
                if conf > 0.6: # Ngưỡng cao hơn cho full-frame
                    self.last_result = f"{label.upper()} ({conf:.1%})"
                    cv2.putText(frame, f"SCENE: {label.upper()}", (20, 40), 
        ...
        # Chuyển đổi để hiển thị lên UI
        rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        img = Image.fromarray(rgb).resize((640, 480))

        if self.ctk_cam_image:
